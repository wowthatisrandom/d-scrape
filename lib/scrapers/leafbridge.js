    ...
    return page.evaluate(() => {
      ...
      cards.forEach(card => {
        try {
          ...
          // Get strain type. Leafbridge markup:
          //   <div class="leafbridge_strain_type_label green_label lf-common">
          //     <div class="leafbridge_strain_type_label_indicator"></div>
          //     <span>Hybrid</span>
          //   </div>
          // The actual label ("Hybrid", "Indica Hybrid", etc.) is in the
          // inner <span>, so read textContent of the wrapper directly.
          // Skip the _indicator child since it matches the same class
          // prefix; prefer the _label wrapper.
          let strainType = null;
          const strainEl = card.querySelector('.leafbridge_strain_type_label, [class*="leafbridge_strain_type_label "], [class*="leafbridge_strain_type_label"]');
          if (strainEl) {
            const text = (strainEl.textContent || '').trim();
            if (text && /^(indica\s*hybrid|sativa\s*hybrid|indica\s*dominant|sativa\s*dominant|hybrid|indica|sativa|cbd)$/i.test(text)) {
              strainType = text;
            }
          }
          // Fallback: some themes render the label elsewhere
          if (!strainType) {
            const strainMatch = fullText.match(/\b(Indica\s*Hybrid|Sativa\s*Hybrid|Indica\s*Dominant|Sativa\s*Dominant|Hybrid|Indica|Sativa)\b/i);
