    ...
    return page.evaluate(() => {
      ...
      productElements.forEach(el => {
        try {
          ...
          // Extract strain type (Indica / Sativa / Hybrid / *-Dominant).
          // This used to be written into `category` as a fallback, which was
          // wrong — it polluted scraped_category with effect values. Now it
          // flows into its own `strainType` field and `effect_type` downstream.
          let strainType = null;
          const textLower = fullText.toLowerCase();
          if (textLower.includes('indica dominant')) strainType = 'Indica Dominant';
          else if (textLower.includes('sativa dominant')) strainType = 'Sativa Dominant';
          else if (textLower.includes('indica')) strainType = 'Indica';
          else if (textLower.includes('sativa')) strainType = 'Sativa';
          else if (textLower.includes('hybrid')) strainType = 'Hybrid';

          // Infer a real product category from the name keywords. Mirrors the
          // corrected logic in the Shift-Mate store locator:
          //   - sesh stick  → Vape
          //   - disposable / vape → Vape
          //   - "hash cone" (literal) → Hash Cones
          //   - cold cure / badder / live rosin → Live Rosin
          //   - syrup / liquid gold / hot sauce → Edible
          // Order matters: sesh stick first so "rosin" keywords don't win.
          let category = null;
