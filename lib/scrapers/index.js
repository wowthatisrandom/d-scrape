...
async function scrapeDispensaryWithDiscovery(dispensary, options = {}) {
  ...
  if (originalConfig?.store_slug) {
    console.log(`  🔄 Retrying with saved store_slug first: ${originalConfig.store_slug}`);
    // Keep store_slug but clear url_format to force direct-first logic
    dispensary.scrape_config = { store_slug: originalConfig.store_slug, brand_slug: originalConfig.brand_slug };
  } else {
    // No store_slug - re-run discovery so Dutchie can attempt:
    // slug discovery -> direct Dutchie -> all remaining formats
    console.log(`  🔍 Retrying with discovery...`);
    dispensary.scrape_config = null;
    scraperOptions = platform === 'dutchie'
      ? { ...options }
      : { ...options, maxFormats: 2 };
  }

  const scraper = getScraper(platform, scraperOptions);
  const result = await scraper.scrape(dispensary);
  ...
  // Handle both old format (array) and new format ({ products, detectedFormat })
  let products, detectedFormat, configWorked, brandSlug, storeSlug;
  if (Array.isArray(result)) {
    products = result;
    detectedFormat = null;
    configWorked = false;
  ...
  // Save the new config if discovery found a working path
  if (products.length > 0 && (detectedFormat || storeSlug) && !configWorked) {
    const configToSave = {};
    if (platform === 'treez') {
      configToSave.brand_slug = detectedFormat;
    } else {
      if (detectedFormat) configToSave.url_format = detectedFormat;
      if (brandSlug) configToSave.brand_slug = brandSlug;
      if (storeSlug) configToSave.store_slug = storeSlug;
    }

    console.log(`  💾 Saving new config: ${JSON.stringify(configToSave)}`);
    try {
      await updateScrapeConfig(dispensary.id, configToSave);
    } catch (e) {
      console.log(`  ⚠️ Failed to save config: ${e.message}`);
