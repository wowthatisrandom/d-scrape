...
class DutchieScraper extends BaseScraper {
  ...
  filterProducts(products) {
    ...
    return products.filter(product => {
      ...
      // Must have "ace solventless" or "ace" as a standalone brand
      // Be strict - only accept if it's clearly Ace Solventless
      if (brand.includes('ace solventless')) {
        return true;
      }

      // Accept if brand is exactly "ace" (common abbreviation)
      if (brand === 'ace') {
        return true;
      }

      // Check product name for "ace solventless" or known Ace products
      if (name.includes('ace solventless')) {
        return true;
      }

      // Known Ace product keywords (Jackpot is their edible line)
      if (name.includes('jackpot') && name.includes('liquid gold')) {
        return true;
      }

      // Cold cure live rosin is Ace's signature product
      if (name.includes('cold cure') && name.includes('live rosin')) {
        return true;
      }
