...
class DutchieScraper extends BaseScraper {
  ...
  async scrape(dispensary) {
    ...
    try {
      ...
      if (savedConfig?.store_slug && !this.rediscover) {
        ...
        try {
          ...
          if (products.length > 0) {
            ...
            console.log(`  ℹ️ Direct URL worked, no Ace products at this location`);
          }
      ...
      if (savedConfig?.url_format && !savedConfig?.store_slug && !this.rediscover) {
        ...
        try {
          ...
          if (result.products.length > 0) {
            console.log(`  ✅ Found ${result.products.length} Ace products with saved config`);
            // Extract and return store_slug for saving
            const storeSlug = result.iframeSrc ? this.extractStoreSlug(result.iframeSrc) : null;
            return {
          ...
          const storeSlug = result.iframeSrc ? this.extractStoreSlug(result.iframeSrc) : null;
          return { products: [], detectedFormat: savedConfig.url_format, storeSlug, configWorked: true };
        } catch (e) {
      ...
      let firstWorkingStoreSlug = null;
      ...
      for (const { url, type } of urlFormats) {
        ...
        try {
          ...
          if (!firstWorkingFormat) {
            ...
            firstWorkingStoreSlug = result.iframeSrc ? this.extractStoreSlug(result.iframeSrc) : null;
          }

          if (result.products.length > 0) {
            const storeSlug = result.iframeSrc ? this.extractStoreSlug(result.iframeSrc) : null;
            console.log(`  ✅ Found ${result.products.length} Ace products with ${type} format (brand: ${result.usedBrandSlug}${storeSlug ? ', store: ' + storeSlug : ''})`);
            return {
              products: result.products,
              detectedFormat: type,
              brandSlug: result.usedBrandSlug,
      ...
      if (firstWorkingFormat) {
        ...
        return { products: [], detectedFormat: firstWorkingFormat, brandSlug: firstWorkingBrandSlug, storeSlug: firstWorkingStoreSlug, configWorked: false };
      }
