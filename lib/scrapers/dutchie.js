    ...
    try {
      ...
      if (savedConfig?.url_format) {
        ...
        try {
          ...
          // Trust saved config - return empty but flag for potential retry
          console.log(`  ℹ️ Saved config returned no products (will retry later if needed)`);
          return { products: [], detectedFormat: savedConfig.url_format, configWorked: true, needsRetry: true };
        } catch (e) {
