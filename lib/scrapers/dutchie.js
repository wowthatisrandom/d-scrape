    ...
    try {
      ...
      if (savedConfig?.store_slug && !this.rediscover) {
        ...
        try {
          ...
          console.log(`  ⚠️ Direct URL failed: ${e.message}, trying other methods...`);
        }
      ...
      // ── PRIORITY 2: filtered_menu_url (user-provided pre-filtered URL) ──
      if (dispensary.filtered_menu_url) {
        console.log(`  🎯 Using filtered_menu_url: ${dispensary.filtered_menu_url}`);
        triedUrls.add(dispensary.filtered_menu_url);
        try {
      ...
      // ── PRIORITY 3: Saved url_format config (legacy iframe path) ──
      ...
      // ── PRIORITY 4: Format discovery (try all URL patterns) ──
