...
class DutchieScraper extends BaseScraper {
   ...
   * Order: plus-search (broadest) -> iframe -> plus-brands (most restrictive)
   */
  getFilteredUrls(baseUrl) {
    ...
    // Format 2: Standard iframe dtche[] params
    ...
    // Format 3: Dutchie Plus with brands=ace-solventless (most restrictive)
    const plusUrl2 = new URL(baseUrl);
    plusUrl2.pathname = basePathname + '/products';
    plusUrl2.search = '';
    plusUrl2.searchParams.set('brands', 'ace-solventless');
    urls.push({ url: plusUrl2.toString(), type: 'plus-brands' });
