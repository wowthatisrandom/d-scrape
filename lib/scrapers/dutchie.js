    ...
    return targetFrame.evaluate(() => {
      ...
      productElements.forEach(el => {
        try {
          ...
          let size = null;
          if (sizeEl) {
            // Get just the size value, not price or "Add to cart"
            const sizeText = sizeEl.textContent || '';
            const sizeMatch = sizeText.match(/^[\d.]+\s*(g|oz|mg|ml)/i);
    ...
    try {
      ...
      if (savedConfig?.url_format) {
        ...
        try {
          ...
          console.log(`  ⚠️ Saved config returned no products, trying all formats...`);
        } catch (e) {
