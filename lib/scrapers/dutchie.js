...
class DutchieScraper extends BaseScraper {
    ...
    return targetFrame.evaluate(() => {
      ...
      productElements.forEach(el => {
        try {
          ...
          // Try multiple brand selectors
          let brandEl = el.querySelector(
            '[class*="full-card__Brand"], [class*="ProductBrand"], [data-testid="product-brand"], [class*="brand"], [class*="Brand"]'
          );
          let brand = brandEl?.textContent?.trim();

          // Brand might be in a span/div below the name
          if (!brand) {
            const spans = el.querySelectorAll('span, div');
            for (const span of spans) {
              const text = span.textContent?.trim()?.toLowerCase();
              if (text?.includes('ace') || text?.includes('solventless')) {
                brand = span.textContent?.trim();
                break;
              }
            }
          }
  ...
  filterProducts(products) {
    ...
    return products.filter(product => {
      ...
      // BLOCKLIST: Exclude known non-Ace products that get incorrectly scraped
      // O.pen brand products start with "O." or "0."
      if (name.startsWith('o.') || name.startsWith('0.')) {
        return false;
      }
      // Solar Flare is O.pen's product line
      if (name.includes('solar flare')) {
        return false;
      }
      // Distillate products are not Ace (they make solventless only)
      if (name.includes('distillate')) {
        return false;
      }

      // ALLOWLIST: Known Ace product patterns
      // Jackpot is their edible line
      if (name.includes('jackpot')) {
        return true;
      }
      // Cold cure live rosin is Ace's signature product
      if (name.includes('cold cure')) {
        return true;
      }
      // Live rosin products
      if (name.includes('live rosin')) {
        return true;
      }
      // Badder products
      if (name.includes('badder')) {
        return true;
      }
      // Hash/sesh stick products
      if (name.includes('hash') || name.includes('sesh stick')) {
        return true;
      }
      // Disposable vapes
      if (name.includes('disposable')) {
        return true;
      }

      // Check brand as fallback
