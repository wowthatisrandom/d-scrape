...
class DutchieScraper extends BaseScraper {
  ...
  getFilteredUrls(baseUrl) {
    ...
    // Format 1: Standard iframe dtche[] params
    const iframeUrl = new URL(baseUrl);
    iframeUrl.searchParams.set('dtche[path]', 'products');
    iframeUrl.searchParams.set('dtche[brands]', 'ace-solventless');
    iframeUrl.searchParams.set('dtche[search]', 'ace');
    urls.push({ url: iframeUrl.toString(), type: 'iframe' });

    // Format 2: Dutchie Plus with brands=ace-solventless
    const plusUrl1 = new URL(baseUrl);
    plusUrl1.pathname = basePathname + '/products';
    plusUrl1.search = '';
    plusUrl1.searchParams.set('brands', 'ace-solventless');
    urls.push({ url: plusUrl1.toString(), type: 'plus-brands' });

    // Format 3: Dutchie Plus with search=ace (broadest)
    const plusUrl2 = new URL(baseUrl);
    plusUrl2.pathname = basePathname + '/products';
    plusUrl2.search = '';
    plusUrl2.searchParams.set('search', 'ace');
    urls.push({ url: plusUrl2.toString(), type: 'plus-search' });
  ...
  async scrape(dispensary) {
    ...
    try {
      ...
      // Try multiple URL formats for filtered results
      const urlFormats = this.getFilteredUrls(dispensary.menu_url);
