...
class DutchieScraper extends BaseScraper {
  ...
  /**
   * Run a Dutchie persisted GraphQL query from the browser context. Dutchie
   * blocks plain Node fetches, but same-origin browser requests work once the
   * embedded menu has loaded.
   */
  async dutchieGraphql(target, basePath, operationName, variables, sha256Hash) {
    return target.evaluate(async ({ basePath, operationName, variables, sha256Hash }) => {
      const url = `${basePath}?operationName=${encodeURIComponent(operationName)}` +
      ...
      return json;
    }, { basePath, operationName, variables, sha256Hash });
  }

  inferCategoryFromName(name, fallbackCategory = null) {
    const nameLower = (name || '').toLowerCase();
    if (nameLower.includes('sesh stick') || nameLower.includes('disposable') || nameLower.includes('vape')) {
      return 'Vape';
    }
    if (nameLower.includes('hash cone')) {
      return 'Infused Preroll';
    }
    if (nameLower.includes('cold cure') || nameLower.includes('badder') || nameLower.includes('live rosin')) {
      return 'Live Rosin';
    }
    if (nameLower.includes('syrup') || nameLower.includes('liquid gold') || nameLower.includes('hot sauce')) {
      return 'Edible';
    }
    return fallbackCategory;
  }

  mapDutchieApiProduct(product, storeSlug) {
    const name = product.Name || product.name || null;
    const brand = product.brand?.name || product.brandName || product.Brand?.name || product.Brand || null;
    const price = product.recPrices?.[0] ?? product.Prices?.[0] ?? product.medicalPrices?.[0] ?? null;
    const slug = product.cName || product.slug || product.Slug || product.CName || null;
    const url = (slug && storeSlug) ? `https://dutchie.com/embedded-menu/${storeSlug}/product/${slug}` : null;
    const size = Array.isArray(product.Options) && product.Options.length > 0 ? product.Options[0] : null;

    return {
      name,
      brand,
      price,
      category: this.inferCategoryFromName(name, product.type || product.Type || product.category || null),
      strainType: product.strainType || product.StrainType || null,
      size,
      url,
      image: product.Image || product.images?.find(img => img.active)?.url || product.images?.[0]?.url || null,
      raw: {
        source: 'dutchie-api',
        id: product._id || product.id || null,
        cName: slug,
        originalName: name,
        // Dutchie reports potency as a number PLUS a unit, and some menus publish
        // milligrams rather than a percentage. Keeping only the number made a
        // 787.8mg 2g preroll indistinguishable from "787.8%" downstream, so carry
        // the unit through. Absent on menus that don't send it — consumers should
        // fall back to the fact that THC cannot exceed 100%.
        thc: product.THCContent?.range?.[0] ?? product.THC ?? null,
        thcUnit: product.THCContent?.unit ?? null,
        cbd: product.CBDContent?.range?.[0] ?? product.CBD ?? null,
        cbdUnit: product.CBDContent?.unit ?? null
      }
    };
  }

  async fetchBrandProductsFromDutchieApi(page, storeSlug) {
    const dispensaryResponse = await this.dutchieGraphql(
      page,
      '/graphql',
      'ConsumerDispensaries',
      { dispensaryFilter: { cNameOrID: storeSlug } },
      '1a669394db4149fe474f55d0b4eba7850460f6d6e748fb27c206ab335db17f92'
    );
    const dispensary = dispensaryResponse?.data?.filteredDispensaries?.[0];
    if (!dispensary?.id) {
      return [];
    }

    const filtersResponse = await this.dutchieGraphql(
      page,
      '/api-4/graphql',
      'MenuFiltersV2',
      {
      ...
      '2f0b3233b8a2426b391649ca3f0f7a5d43b9aefd683f6286d7261a2517e3568e'
    );

    const brands = filtersResponse?.data?.menuFiltersV2?.brands || [];
    const aceBrandIds = brands
      .filter(brand => /\bace\b/i.test(brand.name || ''))
      .map(brand => brand.id)
      .filter(Boolean);

    if (aceBrandIds.length === 0) {
      return [];
    }
    ...
    for (let pageNumber = 0; pageNumber < 10; pageNumber++) {
      const productsResponse = await this.dutchieGraphql(
        page,
        '/api-4/graphql',
        'FilteredProducts',
        {
        ...
        '0fae0c65b4f7910948ee6bb3ff5dc31e5a437839609b037f7d88a5215c82768d'
      );

      const batch = productsResponse?.data?.filteredProducts?.products || [];
      for (const product of batch) {
        const id = product._id || product.id || product.Name || product.name;
        if (id && !seenIds.has(id)) {
          seenIds.add(id);
          products.push(this.mapDutchieApiProduct(product, storeSlug));
        }
      }
  ...
  async useApiProductsIfBetter(page, storeSlug, products) {
    if (!storeSlug) return products;

    try {
      const apiProducts = await this.fetchBrandProductsFromDutchieApi(page, storeSlug);
      if (apiProducts.length > products.length) {
        console.log(`  🔎 Dutchie API returned ${apiProducts.length} Ace products (DOM had ${products.length}); using API results`);
        return apiProducts;
      }
    } catch (error) {
      console.log(`  ⚠️ Dutchie API product fallback failed: ${error.message}`);
    }
  ...
  /**
   * Detect a Dutchie Plus menu URL of the form https://<host>/stores/<slug>(...)
   * Returns the slug or null. This covers both dutchie.com and custom-branded
   * domains (e.g. missourimademarijuana.com), which serve the same Plus SPA.
   * Plus URLs have no iframe, so we can address the brand page directly without
   * the iframe-based slug discovery that always fails (and wastes ~minutes) on
   * these stores.
   */
  extractPlusStoreSlug(url) {
    if (!url) return null;
    const match = url.match(/^https?:\/\/[^/]+\/stores\/([^/?#]+)/);
    return match ? match[1] : null;
  }
  ...
  async tryDirectStoreSlug(page, storeSlug, preferredBrandSlug, apiData, triedUrls, contextLabel = 'Direct URL') {
    ...
    for (const brandSlug of this.getDirectBrandSlugCandidates(preferredBrandSlug || 'ace-solventless')) {
      ...
      try {
        ...
        products = await this.useApiProductsIfBetter(page, storeSlug, products);
  ...
  /**
   * Reconcile DOM-scraped products with the brand products captured from the
   * page's own FilteredProducts GraphQL response (via setupApiInterceptor).
   *
   * On modern Dutchie Plus / custom-domain menus the DOM markup frequently
   * yields 0 products (stale selectors), but the page has already fetched the
   * brand-filtered products over its own — unblocked — GraphQL endpoint. In
   * that case build the result straight from the captured API payload via
   * mapDutchieApiProduct instead of discarding it. When the DOM set is at least
   * as complete, fall back to the existing price-enrichment behavior.
   */
  combineWithApiProducts(domProducts, apiProducts, storeSlug = null) {
    if (apiProducts && apiProducts.length > 0) {
      const mapped = this.filterProducts(
        apiProducts.map(product => this.mapDutchieApiProduct(product, storeSlug))
      );
      if (mapped.length > domProducts.length) {
        console.log(`  🔎 Using ${mapped.length} products from intercepted FilteredProducts API (DOM had ${domProducts.length})`);
        return mapped;
      }
    }
    return this.enrichWithApiPrices(domProducts, apiProducts);
  }
  ...
  async tryScrapeUrl(page, url, formatType = null, options = {}) {
    ...
    const activeStoreSlug = this.extractStoreSlug(page.url()) || this.extractStoreSlug(iframeSrc);
    const apiFetchTarget = targetFrame !== page && activeStoreSlug ? targetFrame : page;
    products = await this.useApiProductsIfBetter(apiFetchTarget, activeStoreSlug, products);
    ...
    if (products.length === 0 && usedDirectFallback && canTryDirect) {
      ...
      if (this.markUrlTried(triedUrls, directUrl)) {
        ...
        products = await this.extractProducts(page);
        products = this.filterProducts(products);
        products = await this.useApiProductsIfBetter(page, this.extractStoreSlug(page.url()) || this.extractStoreSlug(iframeSrc), products);
      } else {
  ...
  async scrape(dispensary) {
    ...
    try {
      ...
      // Best-effort store slug from the menu URL (works for both /stores/<slug>
      // custom-domain Plus menus and dutchie.com/embedded-menu/<slug>), used to
      // label products built from the intercepted FilteredProducts payload.
      const menuSlug = (dispensary.menu_url || '').match(/\/(?:stores|embedded-menu)\/([^/?#]+)/)?.[1]
        || savedStoreSlug
        || null;
      ...
      if (savedPlusFormat && !this.rediscover) {
        ...
        try {
          ...
          const combined = this.combineWithApiProducts(result.products, apiData.products, menuSlug);
          if (combined.length > 0) {
            console.log(`  ✅ Found ${combined.length} Ace products with saved plus config`);
            return {
              products: combined,
      ...
      // When menu_url is a Dutchie Plus URL (<host>/stores/<slug>, including
      // custom-branded domains) we can hit the brand page directly without the
      // iframe-based slug discovery,
      ...
      if (plusSlug) {
        ...
        for (const fmt of plusFastFormats) {
          ...
          try {
            ...
            const combined = this.combineWithApiProducts(result.products, apiData.products, plusSlug || menuSlug);
            if (combined.length > 0) {
              console.log(`  ✅ Found ${combined.length} Ace products with plus fast-path (${fmt})`);
              return {
                products: combined,
                detectedFormat: fmt,
                brandSlug: result.usedBrandSlug,
      ...
      if (dispensary.filtered_menu_url) {
        ...
        if (triedUrls.has(filteredUrl)) {
          ...
          try {
            ...
            const combined = this.combineWithApiProducts(result.products, apiData.products, menuSlug);
            if (combined.length > 0) {
              console.log(`  ✅ Found ${combined.length} Ace products with filtered_menu_url`);
              return {
                products: combined,
      ...
      if (savedConfig?.url_format && !savedStoreSlug && !this.rediscover) {
        ...
        if (triedUrls.has(this.normalizeAttemptUrl(url))) {
          ...
          try {
            ...
            const combined = this.combineWithApiProducts(result.products, apiData.products, menuSlug);
            if (combined.length > 0) {
              console.log(`  ✅ Found ${combined.length} Ace products with saved config`);
              return {
                products: combined,
      ...
      for (const { url, type } of urlFormats) {
        ...
        try {
          ...
          const combined = this.combineWithApiProducts(result.products, apiData.products, menuSlug);
          if (combined.length > 0) {
            console.log(`  ✅ Found ${combined.length} Ace products with ${type} format (brand: ${result.usedBrandSlug})`);
            return {
              products: combined,
              detectedFormat: type,
              brandSlug: result.usedBrandSlug,
      ...
      // Safety net: DOM extraction came up empty on every format, but the page
      // may have fetched brand products over its own GraphQL endpoint that the
      // interceptor captured (modern Plus / custom-domain menus). Use them
      // rather than reporting zero.
      const apiOnly = this.combineWithApiProducts([], apiData.products, menuSlug);
      if (apiOnly.length > 0) {
        console.log(`  ✅ Found ${apiOnly.length} Ace products from intercepted API (DOM empty on all formats)`);
        return {
          products: apiOnly,
          detectedFormat: firstWorkingFormat || 'api-intercept',
          brandSlug: firstWorkingBrandSlug || savedBrandSlug,
          configWorked: false
        };
      }
