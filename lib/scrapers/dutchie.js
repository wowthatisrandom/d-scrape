...
class DutchieScraper extends BaseScraper {
  ...
  async tryScrapeUrl(page, url, formatType = null) {
    ...
    return { products, usedBrandSlug: products.length > 0 ? usedBrandSlug : null, usedDirectFallback };
  }
    ...
    try {
      ...
      for (const { url, type } of urlFormats) {
        ...
        try {
          ...
          if (result.products.length > 0) {
            console.log(`  ✅ Found ${result.products.length} Ace products with ${type} format (brand: ${result.usedBrandSlug})`);
            return {
              products: result.products,
              detectedFormat: type,
              brandSlug: result.usedBrandSlug,
