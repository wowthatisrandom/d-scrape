...
class DutchieScraper extends BaseScraper {
  ...
  addAceFiltersToDirectUrl(url) {
    ...
    if (pathname.includes('/carousels/')) {
      ...
      if (match) {
        pathname = match[1] + '/brands/ace-solventless';
  ...
  async tryScrapeUrl(page, url) {
    ...
    if (dutchieFrame) {
      ...
      if (iframeEmpty && iframeSrc && (iframeSrc.includes('dutchie.com') || iframeSrc.includes('dtche'))) {
    ...
    if (products.length === 0 && dutchieFrame && iframeSrc && (iframeSrc.includes('dutchie.com') || iframeSrc.includes('dtche')) && targetFrame !== page) {
      const directUrl = this.addAceFiltersToDirectUrl(iframeSrc);
