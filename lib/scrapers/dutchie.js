...
class DutchieScraper extends BaseScraper {
  ...
  async tryScrapeUrl(page, url) {
    ...
    const productSelector = '[data-testid="product-list-item"], [data-testid="product-card"], [class*="full-card__Wrapper"], [class*="card__Wrapper"], [class*="ProductCard"], [class*="product-card"], a[href*="/product/"]';
    try {
      await targetFrame.waitForSelector(productSelector, { timeout: 20000 });
      console.log(`  ✅ Product selector found`);
    } catch (e) {
      console.log(`  ⚠️ Product selector not found within timeout`);
    ...
    console.log(`  📦 Extracted ${products.length} products before filtering`);
