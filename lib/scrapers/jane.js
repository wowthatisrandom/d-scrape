      ...
      cards.forEach(card => {
        try {
          ...
          for (let i = 0; i < Math.min(lines.length, 4); i++) {
            ...
            // Skip discount/sale badges like "20% Off Select ACE"
            if (/\d+%\s*off/i.test(line)) continue;
          ...
          // Fallback: the line-split strain-type check only catches cards
          // where "Hybrid"/"Indica"/"Sativa" appears as its own exact line.
          // Many Jane themes render it inline or in a chip/pill element, so
          // scan the full card text for a strain-type word as a secondary
          // extraction. Mirrors the same fallback used in treez.js and
          // dutchie.js.
          if (!strainType) {
            const strainMatch = text.match(/\b(Indica\s*Hybrid|Sativa\s*Hybrid|Indica\s*Dominant|Sativa\s*Dominant|Hybrid|Indica|Sativa)\b/i);
            if (strainMatch) strainType = strainMatch[1];
          }

          // Name-keyword category inference. Order matters: sesh stick
          // before "cold cure / badder / live rosin" because sesh stick
          // names often contain "rosin". Literal "hash cone" only.
          if (!category && name) {
            ...
            if (nameLower.includes('sesh stick') || nameLower.includes('disposable') || nameLower.includes('vape')) {
              ...
              category = 'Live Rosin';
            } else if (nameLower.includes('syrup') || nameLower.includes('liquid gold') || nameLower.includes('hot sauce') || nameLower.includes('jackpot') || nameLower.includes('sudden death')) {
              category = 'Edible';
            }
          }
