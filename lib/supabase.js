...
async function upsertProductAvailability(dispensaryId, products) {
  ...
  const { data: existingProducts, error: fetchError } = await supabase
    ...
    .select('id, scraped_product_name, scraped_size, in_stock, out_of_stock_since')
  ...
  for (const [key, existing] of existingMap) {
    if (!foundKeys.has(key)) {
      // Product was not in this scrape - mark as out of stock
      if (existing.in_stock) {
        // Only update if it was previously in stock
        const updateData = {
          in_stock: false,
          out_of_stock_since: existing.out_of_stock_since || now // Don't overwrite if already set
        };
