...
async function updateDispensaryStatus(dispensaryId, status, errorMessage, productCount) {
  ...
  // First, get current state to check consecutive zeros
  const { data: current, error: fetchError } = await supabase
    .from('dispensaries')
    .select('last_product_count, consecutive_zero_scrapes')
    .eq('id', dispensaryId)
    .single();
  ...
  const currentZeros = current?.consecutive_zero_scrapes || 0;
  ...
  // Handle product count updates with consecutive zero protection
  if (productCount !== undefined) {
    if (productCount > 0) {
      // Found products - update count and reset zero streak
      updateData.last_product_count = productCount;
      updateData.consecutive_zero_scrapes = 0;
    } else {
      // Zero products - check if we should actually set to 0
      const newZeroStreak = currentZeros + 1;
      updateData.consecutive_zero_scrapes = newZeroStreak;

      if (newZeroStreak >= 2) {
        // Confirmed zero after 2+ consecutive scrapes
        updateData.last_product_count = 0;
        console.log(`  📉 ${newZeroStreak} consecutive zero scrapes - confirmed no products`);
