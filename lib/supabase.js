...
const { isExcludedProduct, isDiscontinuedProduct } = require('./exclusions');
...
function inferCategoryFromNormalizedName(normalizedName) {
  ...
  if (n.includes('hash hole')) return 'Infused Preroll';
...
function inferDefaultSize(normalizedName, existingSize) {
  ...
  if (existingSize) {
    // Fixed-size products always show their canonical size regardless of
    // how the menu writes it (500mg, 0.5g, .15g, 2G, ...)
    if (nameLower.startsWith('jackpot infused syrup')) {
      return '1oz - 150mg';
    }
    if (nameLower.startsWith('sesh stick vape')) {
      return '.5g';
    }
    if (nameLower.startsWith('hash cones')) {
      return '2.5g';
    }
    // Canonicalize casing/format of simple gram sizes: "2G" -> "2g", "0.5g" -> ".5g"
    let size = String(existingSize).trim();
    if (/^[\d.]+\s*(g|mg|oz)$/i.test(size)) {
      size = size.toLowerCase().replace(/\s+/g, '').replace(/^0\./, '.');
    }
    return size;
  }
  ...
  if (nameLower.startsWith('bingo hash hole')) return '2g';
  if (nameLower.startsWith('feco syringe')) return '1g';
...
// Consecutive scrapes a product must be missing before it's marked out of stock.
// A partial miss (other products still came back) is trustworthy after 2. A scrape
// that returned nothing at all is weaker evidence, so it takes 3.
const DEFAULT_MISS_THRESHOLD = 2;
const ZERO_SCRAPE_MISS_THRESHOLD = 3;

// Counter stops here — the alert detector treats 3 as "confirmed gone", and this
// keeps long-retired products from being rewritten on every scrape.
const MAX_TRACKED_MISSES = 3;
...
async function upsertProductAvailability(dispensaryId, products, vocab) {
  ...
  const changes = {
    ...
    markedOutOfStock: [],
    returnedInStock: [],
    likelyScrapeFailure: false  // Scrape found nothing at a dispensary that had products (reported, not acted on)
  };
  ...
  const scrapedProducts = products.flatMap(product => {
    const rawName = product.name;
    const size = product.size || null;
    const normalizedName = normalizeProductName(rawName, size, vocab);

    // Drop discontinued products (matched on normalized name, since raw
    // menu names vary too much to pattern-match reliably).
    if (isDiscontinuedProduct(normalizedName)) {
      console.log(`  🚫 Skipping discontinued product: ${normalizedName} (raw: ${rawName})`);
      return [];
    }
  ...
  //
  // A scrape that finds nothing at a dispensary that previously had products is
  // usually a site problem — but it is also exactly what a brand selling out of
  // its last SKU looks like. We can't tell the two apart from one scrape, so we
  // count the miss either way and demand a longer streak before flipping to out
  // of stock. Skipping the miss count entirely (the old behavior) deadlocked:
  // the counter is only incremented here, so a store whose products all sold out
  // stayed pinned in_stock with consecutive_misses=0 forever.
  const isFullZeroScrape = scrapedProducts.length === 0 && existingMap.size > 0;
  const missThreshold = isFullZeroScrape ? ZERO_SCRAPE_MISS_THRESHOLD : DEFAULT_MISS_THRESHOLD;

  if (isFullZeroScrape) {
    console.log(`⚠️ Scrape returned 0 products but ${existingMap.size} existed - requiring ${missThreshold} consecutive misses before marking out of stock`);
  ...
  for (const [key, existing] of existingMap) {
    ...
    if (currentMisses >= missThreshold && existing.in_stock) {
    ...
    } else if (currentMisses <= MAX_TRACKED_MISSES) {
      // Not yet confirmed (or already out of stock) - advance the counter only,
      // capped so already-retired products stop generating writes.
      if (existing.in_stock) {
        console.log(`  ⚠️ ${existing.scraped_product_name} not found (miss #${currentMisses}/${missThreshold}) - waiting for verification`);
      }
      toUpdate.push({
        id: existing.id,
