...
function filterAceProducts(products) {
  return products.filter(product => {
    ...
    // Check for "ace solventless" (most specific)
    if (searchText.includes('ace solventless')) {
      return true;
    }

    // Check for "ace" as a standalone word (not part of another word like "space" or "palace")
    // Using word boundary check
    const acePattern = /\bace\b/i;
    return acePattern.test(searchText);
  });
}
